...
class Team:
    ...
    def __init__(self):
        ...
        # I created this one before last run. Last run was way worse. Imitation agent. Loading this one for first submit.
        # self.model = torch.jit.load(path.join(path.dirname(path.abspath(__file__)), 'adriana_agentDumb70eOK.pt')) # Submitted and best so far
        # self.model = torch.jit.load(path.join(path.dirname(path.abspath(__file__)), 'adriana_agentExtraDataNotGood.pt'))
...
#     # for chunk in load_recording('my_test.pkl'):
#     #     print('*'*40)
#     #     print(chunk)
