...
class Match:
    ...
    def run(self, team1, team2, num_player=1, max_frames=MAX_FRAMES, max_score=3, record_fn=None, timeout=1e10,
        ...
        for it in range(max_frames):
            ...
            # print(f'soccer_state: {soccer_state}')
            # print(f'_use_graphics: {self._use_graphics}')
            if self._use_graphics:
                ...
                # print(f'team1_images: {team1_images}')
            ...
            dir = "data/images2"
            str_it = '_%05d' % it
            fn = path.join(dir, 'ball' + str_it)

            # Save ball location
            ptx, pty = soccer_state["ball"]["location"][0], soccer_state["ball"]["location"][2]
            with open(fn + '.csv', 'w') as f:
                f.write('%0.1f,%0.1f' % (ptx, pty))

            # global n
            # id = n if n < images_per_track else np.random.randint(0, n + 1)
            # if id < images_per_track:
            # print(f'team1_images: {team1_images}')
            # print(f'team1_images length: {len(team1_images)}')
            for id, im in enumerate(team1_images):
            # for id, (im, ptx, pty) in enumerate(zip(team1_images, soccer_state["ball"]["location"][0], soccer_state["ball"]["location"][2])):
            # for im in team1_images:
                track = "team1_player"+str(id)
                # print(f'ball locationx: {soccer_state["ball"]["location"][0]}')
                # print(f'ball locationy: {soccer_state["ball"]["location"][2]}')
                # print(f'ball location?: {soccer_state["ball"]["location"][1]}')
                # fn = path.join(dir, track + '_%05d' % it)
                # fn = path.join(dir, track + '_%05d' % it)
                fn = path.join(dir, track + str_it)
                Image.fromarray(im).save(fn + '.png')
                # with open(fn + '.csv', 'w') as f:
                #     f.write('%0.1f,%0.1f' % tuple(ptx, pty))
            # n += 1

            for id, im in enumerate(team2_images):
                track = "team2_player"+str(id)
                fn = path.join(dir, track + str_it)
                Image.fromarray(im).save(fn + '.png')
